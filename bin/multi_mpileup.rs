...
use htsops::util::{filelist_to_vec, validate_path};


// Call mpileup and pipe output to bgzip
fn execute(bam_path: &str, bgz_path: &str, ref_fasta_path: &str, positions_path: &str) -> (std::process::Child, std::process::Child) {
    ...
    return (mpileup_process, bgzip_process)
}
...
fn main() {
    ...
    let bam_paths: Vec<String> = filelist_to_vec(bam_paths_file);
    let bgz_paths: Vec<String> = filelist_to_vec(bgz_paths_file);
    ...
    for _ in 0..threads {
        if let Some((bam_path, bgz_path)) = bam_bgz_list.pop_front() {
            let (_mpileup_process, bgzip_process) = execute(&bam_path, &bgz_path, ref_fasta_path, positions_path);
    ...
    while !bam_bgz_list.is_empty() || !bgz_queue.is_empty() {
